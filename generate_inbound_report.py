#!/usr/bin/env python3
"""
Inbound Call Report — Download CSV from CallInsight, build HTML, send via WhatsApp.
Usage:  python generate_inbound_report.py [--local] [--date DD/MM/YYYY] [--csv path]
  --local   skip WhatsApp send, just save the HTML
  --date    override report date (default: today IST)
  --csv     path to existing CSV file (skips download)
"""
...
def process_csv(csv_text: str, target_date: str, from_time: str = None, to_time: str = None):
    """Filter INBOUND + target_date (+ optional time window) → per-counsellor stats."""
    stats = {}
    ...
    for row in reader:
        ...
        if date_val != target_date:
            continue
        if row.get('Call Type', '').strip().upper() != 'INBOUND':
            continue
...
def build_html(stats: dict, date_label: str, csv_source: str = '', time_window: str = '') -> str:
    ...
    teams = {}
    for name, s in stats.items():
        teams.setdefault(s['team'], []).append({'name': name, **s})
  ...
  <table>
    <thead><tr>
      ...
      <th>Pick&nbsp;%</th><th>Talk&nbsp;Time</th><th>Avg&nbsp;/&nbsp;Call</th>
    </tr></thead>
    <tbody>{rows}</tbody>
    <tfoot><tr>
      <td>Team Total</td>
...
<title>Inbound Report · {date_label}</title>
...
<div class="sup-table-wrap">
  ...
  <table>
    <thead><tr>
      ...
      <th>Counsellors</th><th>Calls</th><th>Picked&nbsp;Up</th>
      <th>Pick&nbsp;%</th><th>Talk&nbsp;Time</th><th>Avg&nbsp;/&nbsp;Call</th>
      <th>Talk&nbsp;/&nbsp;Active&nbsp;(5+)</th>
    </tr></thead>
...
<div class="kpis">
  ...
  <div class="kpi-card" style="--ac:#e5e7eb"><div class="v">{fmt_avg(g_avg)}</div><div class="l">Avg / picked</div></div>
...
<footer>Source: {source_note} &nbsp;·&nbsp; Inbound calls only &nbsp;·&nbsp; {date_label}</footer>
</body>
</html>'''
...
async def main():
    ...
    log(f'Parsed: {total_calls} inbound calls, {total_ans} picked up, {active} active counsellors')
    ...
    html_filename = f'Inbound_Report_{eff_stamp}{slug}.html'

...
<div class="sup-bar-bg"><div class="sup-bar" style="width:{bar_width}%"></div></div></div>\n'''
        ...
        # Summary table row
        sup_summary_rows += f'''<tr><td class="left bold">{disp_name}</td><td>{adm_tgt}</td><td>{adm_ach_val}</td><td>{adm_ach_val}/{adm_tgt}</td><td class="rev">{money_full(fee_tgt)}</td><td class="rev">{money_full(fee_ach)}</td><td>{pill(fee_pct_str)}</td><td class="ftd">{money_full(ftd_fee)}</td><td>{num(ftd_adm) if ftd_adm else 0}</td></tr>\n'''
    ...
    sup_summary_rows += f'''
<tr class="grand-total"><td class="left bold">&#9679; Grand Total</td><td>{total_adm_target}</td><td>{gt_adm_ach}</td><td>{gt_adm_ach}/{total_adm_target}</td><td class="rev">{money_full(gt_fee_tgt)}</td><td class="rev">{money_full(gt_fee_ach)}</td><td>{pill(gt_fee_pct_str)}</td><td class="ftd">{money_full(gt_fee_ftd)}</td><td>{num(gt_adm_ftd)}</td></tr>'''
...
'''
    ...
    p2 = f'<div class="shell"><header class="header"><div class="header-top"><div><div class="brand">Performance Intelligence &middot; Online</div><h1 class="title">Online Admissions &amp;<br>Fee Collected Tracker</h1></div><div class="header-meta"><div class="badge">Live Report</div><div class="date">{MONTH_LABEL} &middot; FTD {report_date.strftime("%d %b")}</div></div></div></header>'
...
def regular_generate_html(sheets):
...
#tab-admissions,#tab-forms{display:none}
...
.tabs{display:flex;gap:0;margin-bottom:24px;border:1.5px solid var(--border-dark);border-radius:var(--radius);overflow:hidden}
...
#tab-admissions:checked~.shell .tab-label[for=tab-admissions],#tab-forms:checked~.shell .tab-label[for=tab-forms]{background:var(--ink);color:var(--white)}
.panel{display:none}
#tab-admissions:checked~.shell #panel-admissions,#tab-forms:checked~.shell #panel-forms{display:block}
...
.kpi-label{font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:var(--ink-light);margin-bottom:4px}
...
<div class="shell">
<header class="header"><div class="header-top"><div><div class="brand">Performance Intelligence &middot; Regular</div><h1 class="title">Regular Admissions &amp;<br>Forms Tracker</h1></div><div class="header-meta"><div class="badge">Live Report</div><div class="date">{MONTH_LABEL} &middot; FTD {report_date.strftime('%d %b')}</div></div></div></header>
<div class="tabs"><label class="tab-label" for="tab-admissions">&#x1f393; Admissions</label><label class="tab-label" for="tab-forms">&#x1f4cb; Forms</label></div>
...
<div class="kpi"><div class="kpi-label">{MONTH_SHORT} Ach %</div><div class="kpi-value {pc(admt.iloc[4])}">{pv(admt.iloc[4]):.1f}%</div><div class="kpi-ratio" style="font-size:11px;opacity:0.8">{admt.iloc[3]}/{admt.iloc[2]}</div><div class="kpi-sub">vs {n(admt.iloc[2])} target</div></div>
<div class="kpi"><div class="kpi-label">Week Ach %</div><div class="kpi-value {pc(admt.iloc[7])}">{pv(admt.iloc[7]):.1f}%</div><div class="kpi-ratio" style="font-size:11px;opacity:0.8">{admt.iloc[6]}/{admt.iloc[5]}</div><div class="kpi-sub">{n(admt.iloc[6])} of {n(admt.iloc[5])}</div></div>
</div>
...
<div class="kpi"><div class="kpi-label">{MONTH_SHORT} Ach %</div><div class="kpi-value {pc(formt.iloc[4])}">{pv(formt.iloc[4]):.1f}%</div><div class="kpi-ratio" style="font-size:11px;opacity:0.8">{formt.iloc[3]}/{formt.iloc[2]}</div><div class="kpi-sub">vs {n(formt.iloc[2])} target</div></div>
<div class="kpi"><div class="kpi-label">Week Ach %</div><div class="kpi-value {pc(formt.iloc[7])}">{pv(formt.iloc[7]):.1f}%</div><div class="kpi-ratio" style="font-size:11px;opacity:0.8">{formt.iloc[6]}/{formt.iloc[5]}</div><div class="kpi-sub">{n(formt.iloc[6])} of {n(formt.iloc[5])}</div></div>
</div>
<div class="section-label"><span>College-wise Breakdown</span></div>

...
"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DAILY FORMS DUMP (ONLINE) → GOOGLE SHEET
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Queries the ONLINE LMS database for yesterday's first-form submissions
and writes the results to the tracking sheet.

  Target Sheet: https://docs.google.com/spreadsheets/d/1pFq4-ElGJ81y7SiaHGA6HspEpDicf93NPsPZ2k32hYU
  Target Tab:   gid=0

Usage:  python dump_forms_to_sheet_online.py [YYYY-MM-DD]
        Defaults to yesterday (IST) if no date given.

Columns written (in order): TBD — placeholder, update once query is provided
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
# ─── DB CONFIG — Single Online LMS DB ──────────────────────────────────────────
DB_CONFIG = {
...
WITH first_form AS (
    SELECT
        csj.student_id,
        csj.course_id,
        MIN(csj.created_at) AS first_form_date
...
LEFT JOIN counsellors l3_c ON s.assigned_counsellor_l3_id = l3_c.counsellor_id
...
async def fetch_all():
    """Fetch from the single Online LMS DB."""
    all_rows = []
    conn = await asyncpg.connect(**DB_CONFIG)
    try:
        rows = await conn.fetch(ONLINE_FORMS_QUERY, datetime.strptime(REPORT_DATE_STR, '%Y-%m-%d').date())
        count = len(rows)
        print(f"  [ONLINE] {count} rows")
        for r in rows:
            d = dict(r)
            all_rows.append(row_to_sheet(d))
    finally:
        await conn.close()
    return all_rows
...
async def main():
    print("=" * 60)
    print("📊 DAILY FORMS DUMP (ONLINE) → GOOGLE SHEET")
    print("=" * 60)
    print(f"   Date: {REPORT_DATE_STR}")
    print()

    print("─── Fetching from Online LMS database ─────────────────────────────")
